...
from matplotlib.ticker import MultipleLocator, MaxNLocator, AutoLocator, ScalarFormatter, LinearLocator, FixedFormatter, \
    AutoMinorLocator
...
from shapely import get_geometry, symmetric_difference, remove_repeated_points, get_coordinates, Polygon, \
    get_exterior_ring, transform, centroid, get_x, get_y
...
class Plot:
    ...
    def plot_polygons(self,color,color_2,ax=None,fig=None):
        ...
        #plt.colorbar.invert_yaxis()

        #norm = mpl.colors.Normalize(vmin=0, vmax=5.6)

        #fig.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(vmin=0, vmax=5.6), cmap=cmap),cax=ax, orientation='vertical', label='Time (s)')
    ...
    def animate(self,name,c_map,save=False):
        ...
        #test = mpl.colormaps[c_map].resampled(len(self.polygons))
        # cmap = plt.get_cmap('viridis')
        #colors = test.colors  # Get all colors in the colormap as an RGBA array
        ...
        def update(frame):
            #tup = (colors[-1-frame][0], colors[-1-frame][1], colors[-1-frame][2])
